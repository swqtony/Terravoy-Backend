-- Rollback: restore match functions before SETOF change

-- public.try_match(uuid)
...
-- public.start_match(uuid, uuid, text, integer, integer, text[], text)
CREATE OR REPLACE FUNCTION public.start_match(p_profile_id uuid, p_trip_card_id uuid, p_preferred_gender text DEFAULT NULL::text, p_preferred_age_min integer DEFAULT NULL::integer, p_preferred_age_max integer DEFAULT NULL::integer, p_preferred_languages text[] DEFAULT NULL::text[], p_city_scope_mode text DEFAULT 'Strict'::text)
 RETURNS match_sessions
 LANGUAGE plpgsql
 SECURITY DEFINER
