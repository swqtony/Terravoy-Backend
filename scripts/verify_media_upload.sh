...
if [[ -z "$FILE" ]]; then
  echo "Set FILE to an image/video path before running (example: FILE=./path/to/file.jpg)."
  exit 1
fi
...
upload_resp=$(
  curl -sS -X POST "$HOST/v1/media/upload-url" \
    ...
    -d "{\"scope\":\"$SCOPE\",\"visibility\":\"$VISIBILITY\",\"ext\":\"$EXT\",\"size\":$size,\"mime\":\"$MIME\"}"
)
echo "$upload_resp"
...
echo "== failure: invalid scope =="
curl -sS -X POST "$HOST/v1/media/upload-url" \
  -H "Content-Type: application/json" \
  -H "x-leancloud-user-id: $LC_USER" \
  -H "x-leancloud-sessiontoken: $LC_SESSION" \
  -d "{\"scope\":\"bad\",\"visibility\":\"public\",\"ext\":\"jpg\",\"size\":123,\"mime\":\"image/jpeg\"}" \
  | cat

echo "== failure: owner mismatch =="
bad_key="public/post/someone_else/2025/12/00000000-0000-0000-0000-000000000000.jpg"
curl -sS -X POST "$HOST/v1/media/complete" \
  -H "Content-Type: application/json" \
  -H "x-leancloud-user-id: $LC_USER" \
  -H "x-leancloud-sessiontoken: $LC_SESSION" \
  -d "{\"objectKey\":\"$bad_key\",\"declaredSize\":123,\"declaredMime\":\"image/jpeg\"}" \
  | cat

echo "== failure: object missing =="
missing_key="public/post/$LC_USER/2025/12/00000000-0000-0000-0000-000000000001.jpg"
curl -sS -X POST "$HOST/v1/media/complete" \
  -H "Content-Type: application/json" \
  -H "x-leancloud-user-id: $LC_USER" \
  -H "x-leancloud-sessiontoken: $LC_SESSION" \
  -d "{\"objectKey\":\"$missing_key\",\"declaredSize\":123,\"declaredMime\":\"image/jpeg\"}" \
  | cat
