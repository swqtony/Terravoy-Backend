-- Final cleanup for ensure_profile overload conflicts.
-- Goal: leave ONLY ensure_profile_v2(text, uuid default null) so PostgREST can resolve the RPC.

-- Drop every legacy overload to avoid PGRST203.
...
-- Recreate the canonical RPC with a unique name/signature.
DROP FUNCTION IF EXISTS public.ensure_profile_v2(text);
