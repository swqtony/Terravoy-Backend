...
const STATUS_SET = new Set(['draft', 'published', 'paused', 'archived', 'rejected']);
...
export default async function adminExperiencesRoutes(app) {
  const pool = app.pg.pool;
  const requireExperiencesRead = requirePermission('experiences.read', pool);
  const requireExperiencesWrite = requirePermission('experiences.write', pool);

  app.get('/functions/v1/admin/experiences', async (req, reply) => {
    const admin = await requireExperiencesRead(req, reply);
    if (!admin) return;

    const status = normalizeString(req.query?.status || '');
    const hostId = normalizeString(req.query?.hostId || '');
    ...
    if (hostId) {
      params.push(hostId);
      where.push(`host_user_id = $${params.length}`);
    }
    if (city) {
      params.push(city);
    ...
    if (q) {
      params.push(`%${q}%`);
      where.push(
        `(id::text ilike $${params.length} or title ilike $${params.length} or summary ilike $${params.length} or description ilike $${params.length})`
      );
    }
    if (dateFrom) {
    ...
    try {
      const countResult = await pool.query(
        `select count(*)::int as total from experiences ${whereClause}`,
        params
      );
  ...
  app.get('/functions/v1/admin/experiences/:id', async (req, reply) => {
    const admin = await requireExperiencesRead(req, reply);
    if (!admin) return;

    const id = normalizeString(req.params?.id);
    if (!id) {
      return error(reply, 'INVALID_REQUEST', 'Invalid experience id', 400);
    }
  ...
  app.patch('/functions/v1/admin/experiences/:id', async (req, reply) => {
    const admin = await requireExperiencesWrite(req, reply);
    if (!admin) return;
    ...
    try {
      const beforeResult = await pool.query('select * from experiences where id = $1', [id]);
